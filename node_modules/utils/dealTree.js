...
var lastGetDataTime;//最后一次取数据的时间

var config,copyToPath,_log,_logError;
function init (_config) {
	config = _config;

	copyToPath = path.normalize(config.copyToPath);
	_log = logger.prefixLogSync(config.logPath,config.dealLogPrefix);
	_logError = logger.errorSync(config.logPath);
}
...
var _copy = function(sourcePath,targetPath){
	...
	/*当大文件正在写操作时不进行处理*/
	if(fs.statSync(sourcePath).mtime >= lastGetDataTime){
		_logError('copy',sourcePath,'is modifing');
		return;
	}
	if(fs.statSync(sourcePath).size > 0){
		fsUtil.copyFileSync(sourcePath,targetPath);
...
function _dealDeleteTree(deleteTree,base){
	...
	fsUtil.mkdirSync(deleteTreePath);
...
function getDataFromMemory(callback){
	callback || (callback = function(){});
	misc.curl(config.host,config.port,'/',function(err,data){
		if(err){
			_logError('problem with request: ' + JSON.stringify(err));
			callback(err);
		}else{
			lastGetDataTime = new Date();
			try{
